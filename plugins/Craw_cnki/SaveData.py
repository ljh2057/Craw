...
class BlobDataTestor:
    ...
    def getDataFrom(self,filename):
        ...
        for n in range(0, len(ops)):
            # list_data=str(ops[n:n+1]).lstrip('[').rstrip(']')
            # cursor.execute('insert into corpus(DOCID,TITLE,AUTHOR,DEPART,KEYWORD,ABSTRACT,JOURNAL,PT,URL,CRID,SUFFIX,ORIGIN)values%s'%list_data)
            # try:
            # cursor.executemany(
            #     'insert into corpus (DOCID,TITLE,AUTHOR,DEPART,KEYWORD,ABSTRACT,JOURNAL,PT,URL,CRID,SUFFIX,ORIGIN)values (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)',
            #     ops[n:n + 1])
            print(ops[n:n + 1])
            try:
...
#         # test.testRWBlobData()
#         # test.teardown()
#         test.getDataFrom('data/文献属性.xls')
#     finally:
#         test.closedb()