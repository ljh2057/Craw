...
class Baidu(object):
    ...
    def getdetail(self, news_url):
        ...
        # 发布者
        self.poster = \
        bf.select("#detail-page > div.title_border > div > div.article-desc.clearfix > div.author-txt > p")[
            0].get_text()
    ...
    def write_txt(self):
        ...
        self.txtpath = self.filepath.replace('\\', '/') + "/" + txtname + ".txt"
        # print("写入txt：" + txtpath)
        writetxt = open(self.txtpath, "w")
        for i in range(len(self.content_list)):
            writetxt.write(self.content_list[i])
        writetxt.close()

    def write_excel(self):
        self.sheet.write(self.num, 0, self.title)
        self.sheet.write(self.num, 1, self.poster)
        self.sheet.write(self.num, 2, self.CRID)
